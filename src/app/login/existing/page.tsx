...
export default function ExistingLoginPage() {
  ...
  return (
    <div className="mx-auto flex h-dvh w-full max-w-[420px] flex-col px-7 pt-[max(env(safe-area-inset-top),24px)] pb-[max(env(safe-area-inset-bottom),24px)]">
      <div>
        <BackButton href="/login/start" className="mb-12" />
        <p className="mb-5 text-xs font-bold tracking-wide text-ink-faint">오내요</p>
        <h1 className="mb-4 text-[28px] font-bold leading-tight">다시 만나서 반가워요</h1>
        <p className="text-sm text-ink-soft">아이디와 비밀번호를 입력해주세요.</p>
      </div>

      <span className="mt-14 mb-14 text-center text-[96px] leading-none">🤗</span>
